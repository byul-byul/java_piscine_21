...
public class Program {
    ...
    private static String   defineFileSignature(String fPath, int bytesCount) throws IOException {
        ...
        try {
            InputStream inFile = new FileInputStream(fPath);
    ...
    private static String   getFileSignatureAndWriteToOutFile(String fPath, ArrayList<String[]> signs, String outFileName) {
        try {
            OutputStream outStream = new FileOutputStream(outFileName, true);
    ...
    private static ArrayList<String[]>  collectSignaturesFromFile(String inFileName) {
        ...
        try {
            ...
            while (inStream.available() > 0) {
                ...
                if (chr == '\n') {
                    signList.add(new String[]{strBld.substring(strBld.indexOf(",") + 2).replace(" ", ""),
                                                                strBld.substring(0, strBld.indexOf(","))});
                    strBld.setLength(0);
                }
                else {
                    strBld.append(chr);
    ...
    public static void      main(String[] args) {
        ...
        ArrayList<String[]> signatureList = collectSignaturesFromFile(inputFuleName);
